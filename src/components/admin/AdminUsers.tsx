...
const AdminUsers = ({ lang }: { lang: Language }) => {
  const t = adminT[lang];
    ...
    <div dir={dir}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-foreground text-base font-bold flex items-center gap-2">
          <Users className="w-4 h-4 text-muted-foreground" /> {t.userManagement}
        </h2>
            ...
            <div key={user.id} className="bg-card rounded-xl p-4 border border-border flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <div className={`text-xs mt-0.5 ${user.role === 'super' ? 'text-primary' : 'text-blue-400'}`}>
                  {user.role === 'super' ? t.superAdmin : user.role === 'staff' ? t.staff : t.accessDenied}
                </div>
                <div className="flex gap-1.5 mt-2 flex-wrap">
                  {user.role === 'super' ? (
                    [t.fullAccess, t.reports, t.paymentsApi, t.users, t.menuItems].map(p => (
                      <span key={p} className="px-2 py-0.5 rounded text-[10px] bg-primary/10 text-primary font-medium">{p}</span>
                    ))
                  ) : (
                    [t.dailyOrders, t.viewMenu].map(p => (
          ...
          <div className="bg-card border border-border rounded-xl p-6 min-w-[420px] animate-modal-in">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-foreground text-base font-bold flex items-center gap-2">
                <Plus className="w-4 h-4 text-primary" /> {t.addUser}
              </h3>
            ...
            {[
              { label: t.email, value: newUser.email, key: 'email', type: 'email', placeholder: 'user@example.com', icon: Mail },
              { label: t.fullName, value: newUser.fullname, key: 'fullname', type: 'text', placeholder: lang === 'ku' ? 'ناوی تەواو' : lang === 'ar' ? 'الاسم الكامل' : 'Full Name', icon: UserIcon },
              { label: t.password, value: newUser.password, key: 'password', type: 'password', placeholder: '••••••••', icon: KeyRound },
            ].map(f => (
            ...
            <div className="mb-4">
              <label className="text-muted-foreground text-[10px] tracking-widest uppercase block mb-1.5 font-semibold">{t.role}</label>
              <select className="w-full p-2.5 bg-secondary border border-border rounded-lg text-foreground text-sm" value={newUser.role} onChange={e => setNewUser(p => ({ ...p, role: e.target.value as 'super' | 'staff' }))}>
                ...
                <option value="super">{t.superAdmin}</option>
              </select>
            </div>
